...
internals.properties.prototype.parseProperty = function (name, joiObj, parent, parameterType, useDefinitions, isAlt) {
    ...
    if (property.type === 'object') {
        if (Utilities.hasJoiChildren(joiObj)) {
            ...
            // hack a fix for: https://github.com/glennjones/hapi-swagger/issues/360
            // add object pattern properties
            let objectPattern = joiObj._inner.patterns[0];
            let patternHasChildren = objectPattern && Utilities.hasJoiChildren(objectPattern.rule);
            if (patternHasChildren){
               // this part is a little weird. We don't want to append the pattern's children directly to this object. 
               // Instead We want to append a new dummy object based on our dynamic key based pattern. Since it doesn't have a name we'll default to "$key"
               // then we'll append the pattern's children to that (they are in objectPattern.rule._inner.children).
               //let patternRegex = objectPattern.regex;
               let patternName = "$key";
               let patternLabel = objectPattern.rule._flags.label;
               property.properties = {};
               property.properties[ patternName] = this.parseProperty(patternLabel, objectPattern.rule, property, parameterType, useDefinitions, isAlt);
               property.required = [ patternName ];
            }
